...
public sealed class IndexedDbContextTests
{
    ...
    private static MockJsRuntime BuildMock()
    {
        ...
        mock.Returns("IdbBlazor.openDatabase",
    ...
    public async Task EnsureInitializedAsync_CallsOpenDatabase()
    {
        ...
        var call = mock.Calls.FirstOrDefault(c => c.Identifier == "IdbBlazor.openDatabase");
        Assert.NotNull(call);
    }
    ...
    public async Task AddAsync_CallsJsAdd()
    {
        ...
        Assert.Contains(mock.Calls, c => c.Identifier == "IdbBlazor.add");
    }
    ...
    public async Task FindAsync_CallsJsGet_AndDeserializes()
    {
        ...
        mock.Returns("IdbBlazor.get", JsonSerializer.Serialize(expected,
    ...
    public async Task ToListAsync_CallsJsGetAll_AndDeserializes()
    {
        ...
        mock.Returns("IdbBlazor.getAll", SerializeList(data));
        var db = new TestDb(mock);
        await db.EnsureInitializedAsync();
    ...
    public async Task Where_NativeTranslation_CallsGetAllByIndex()
    {
        ...
        mock.Returns("IdbBlazor.getAllByIndex", SerializeList(data));
        var db = new TestDb(mock);
        await db.EnsureInitializedAsync();
        ...
        Assert.Contains(mock.Calls, c => c.Identifier == "IdbBlazor.getAllByIndex");
        Assert.Single(list);
    }
    ...
    {
        var mock = BuildMock();
        mock.Returns("IdbBlazor.delete", true);
        var db = new TestDb(mock);
        await db.EnsureInitializedAsync();

        await db.Products.DeleteAsync(1);

        Assert.Contains(mock.Calls, c => c.Identifier == "IdbBlazor.delete");
    }
    ...
    {
        var mock = BuildMock();
        mock.Returns("IdbBlazor.executeTransaction", "[]");
        var db = new TestDb(mock);
        await db.EnsureInitializedAsync();
        ...
        Assert.Contains(mock.Calls, c => c.Identifier == "IdbBlazor.executeTransaction");
    }
    ...
    {
        var mock = BuildMock();
        mock.Returns("IdbBlazor.count", 42);
        var db = new TestDb(mock);
        await db.EnsureInitializedAsync();
    ...
    public async Task Options_OverridesDatabaseName()
    {
        ...
        var call = mock.Calls.First(c => c.Identifier == "IdbBlazor.openDatabase");
